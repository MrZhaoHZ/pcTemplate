...
* @Date:   2018-01-26 16:06:54
* @Last Modified by:   Administrator
* @Last Modified time: 2018-02-02 10:32:06
*/
		...
		$ajax.ajaxPost('/order/list.do', filterCondition, function(data) {
			if(data.success){
				...
				if(data.data.datas) {
					...
					$("#pager").pager({ pagenumber: pageIndex, pagecount:  Math.ceil(data.data.total_count / filterCondition.page_size), buttonClickCallback: getOrderList, totalCounts:data.data.total_count});
					$("#pager").show();
				} else {
					$('#order-list').html('');
					$("#pager").hide();
